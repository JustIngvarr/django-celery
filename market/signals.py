...
subscription_notice = Signal(providing_args=['instance'])
...
@receiver(subscription_notice, dispatch_uid='notify_student_subscription')
def notify_student_subscription(sender, **kwargs):
    c = kwargs['instance']
    owl = Owl(
        template='mail/class/student/notice.html',
        ctx={
            'c': c,
        },
        to=[c.customer.user.email],
